...
function Input({
  ...
  return (
    <label className="block space-y-2">
      ...
      <div className={cx((isPassword || prefix) && "relative")}>
        {prefix && (
          <span className="absolute left-4 top-1/2 -translate-y-1/2 text-sm font-semibold text-[var(--text-muted)] select-none">
            {prefix}
          </span>
        )}
        <input
        ...
        {isPassword && (
          <button
            ...
            className="absolute right-4 top-1/2 -translate-y-1/2 text-[var(--text-muted)] hover:text-[var(--text)] focus:outline-none transition-colors"
          >
            {showPassword ? (
